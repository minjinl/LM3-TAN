...
use nitrogen_data_mod, only : read_nitrogen_namelist, do_nitrogen
use nitrogen_mod, only : nitrogen_init, n_soil_depth
use nitrogen_sources_mod, only : nitrogen_sources_init, nitrogen_sources_end, &
     nitrogen_deposition, nitrogen_fertilization
...
character(len=*), parameter :: &
     module_name = 'land', &
     version     = '$Id: land_model.F90,v 18.0.2.5.2.1.2.1 2011/10/04 00:03:12 slm Exp $', &
...
10   continue
  ...
  call nitrogen_init ( id_lon, id_lat )
  call nitrogen_sources_init(lnd%time)
...
subroutine update_land_model_fast ( cplr2land, land2cplr )
  ...
  real :: sorg_depth, & ! depth of organic soil for soil nitrogen calculations, m
          sorg_uptake, &  ! uptake from organic soil, kg/(m2 s)
          sorg_drain, & ! drainage of water from organic of soil, kg/(m2 s)
          surf_runoff, & ! surface runoff, kg/(m2 s)
          ndep_nit, ndep_amm, ndep_don, fm_nit, fm_amm, fm_don, &  ! deposition rate for nitrate and ammonium, kg N/(m2 yr)
	  donleach, & ! dissolved organic nitrogen leaching rate, kg N/(m2 s)
	  ammleach, & ! ammonium leaching rate, kg N/(m2 s)
	  nitrleach   ! nitrate leaching rate, kg N/(m2 s)
  ...
  real, dimension(lnd%is:lnd%ie,lnd%js:lnd%je) :: &
       ...
       runoff_don,       & ! runoff of dissolved organic nitrogen, kg N/(m2 year)
       runoff_amm,       & ! runoff of ammonium, kg N/(m2 year)
       runoff_nitr,      & ! runoff of nitrate, kg N/(m2 year)
  ...
  logical :: used          ! return value of send_data diagnostics routine
  real, dimension(lnd%is:lnd%ie,lnd%js:lnd%je) :: &
       crop_fert, past_fert! fertilization amounts, kg N/m2
  real :: nfert, nfm_nit, nfm_amm, nfm_don ! fertilization rate per tile, kg N/(m2 year)
  ...
  real, allocatable :: sewage_data(:,:)  ! buffer for data
  ...
  runoff_don = 0 ; runoff_amm = 0 ; runoff_nitr = 0
  ...
  call nitrogen_fertilization(lnd%time, lnd%time+lnd%dt_fast, crop_fert, past_fert)
...
! returned to the flux exchange.
     ...
     if (associated(tile%glac)) then
        ...
        sorg_depth = n_soil_depth(tile%vegn)
        call soil_step_2 &
             ...
             subs_levap, subs_fevap, &
             subs_melt, subs_lrunf, subs_hlrunf, subs_Ttop, subs_Ctop, &
             sorg_uptake, sorg_drain, surf_runoff )
     endif
...
! ALSO, DIAGNOSTICS IN COMPONENT MODULES SHOULD _FOLLOW_ THIS ADJUSTMENT******
     ...
     if (associated(tile%vegn)) then
        ...
        if (do_nitrogen) &
           call nitrogen_deposition(lnd%time,i,j,tile%vegn%p_ann,precip_l+precip_s,&
                     ndep_nit,ndep_amm,ndep_don,fm_nit,fm_amm,fm_don)
        ! calculate fertilization
        nfert = 0
	nfm_nit = 0
	nfm_amm = 0
	nfm_don = 0
        if (tile%vegn%landuse == LU_CROP) nfert = nfert + crop_fert(i,j)
	if (tile%vegn%landuse == LU_CROP) nfm_nit = nfm_nit + fm_nit
	if (tile%vegn%landuse == LU_CROP) nfm_amm = nfm_amm + fm_amm
	if (tile%vegn%landuse == LU_CROP) nfm_don = nfm_don + fm_don
        if (tile%vegn%landuse == LU_PAST) nfert = nfert + past_fert(i,j)
        ! convert amount (kg N/m2) to rate (kg N /(m2 year)), same as deposition
        nfert = nfert/delta_time*seconds_per_year

        call vegn_step_3 (tile%vegn, tile%soil, tile%cana%prog%T, &
             precip_l+precip_s, ndep_nit, ndep_amm, ndep_don, nfert, nfm_nit, nfm_amm, nfm_don, &
             sorg_depth, sorg_uptake, sorg_drain, &
             surf_runoff, vegn_fco2, donleach, ammleach, nitrleach, tile%diag)
     endif
     ...
     runoff_don(i,j) = runoff_don(i,j) + donleach*tile%frac
     runoff_amm(i,j) = runoff_amm(i,j) + ammleach*tile%frac
     runoff_nitr(i,j) = runoff_nitr(i,j) + nitrleach*tile%frac 
  ...
  if (do_age) runoff_c(:,:,3) = 0
  if (num_c.eq.3) then 
      runoff_c(:,:,num_species-2) = runoff_don
      runoff_c(:,:,num_species-1) = runoff_amm
      runoff_c(:,:,num_species) = runoff_nitr
    endif

! Example of additional dataTable entries (in XML)
 !"LND", "sewage_nit", "INPUT/point_nit.nc", "nit", .true., 1.0
 !"LND", "sewage_amm", "INPUT/point_amm.nc", "amm", .true., 1.0
 !"LND", "sewage_don", "INPUT/point_don.nc", "don", .true., 1.0

  allocate(sewage_data(is:ie,js:je))
  sewage_data(:,:) = 0.0
  call data_override('LND','sewage_nit',sewage_data,lnd%time)
  runoff_c(:,:,num_species) = runoff_c(:,:,num_species)+sewage_data

  sewage_data(:,:) = 0.0
  call data_override('LND','sewage_amm',sewage_data,lnd%time)
  runoff_c(:,:,num_species-1) = runoff_c(:,:,num_species-1)+sewage_data

  sewage_data(:,:) = 0.0
  call data_override('LND','sewage_don',sewage_data,lnd%time)
  runoff_c(:,:,num_species-2) = runoff_c(:,:,num_species-2)+sewage_data
!=================================================================================
  ! update river state
