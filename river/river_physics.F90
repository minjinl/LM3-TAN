...
!--- version information ---------------------------------------------
  character(len=128) :: version = '$Id: river_physics.F90,v 18.0.2.1 2010/04/30 00:57:25 slm Exp $'
        ...
        if (River%travel(i,j)==cur_travel.and.&
            ...
            if (River%tocell(i,j).eq.0 .and. River%landfrac(i,j).ge.1.) then
                ...
                if (lake_area.gt.0.) then
                     ...
                     River%lake_outflow_c(i,j,:) = influx_c
...
!                           ( clw*River%storage(i,j) + (csw-clw)*River%storage_c(i,j,1))
		...
		!temperature(i,j)=min(300,max(270,temperature(i,j)))
		    ...
		    River%removal_c(i,j,3) = River%storage_c(i,j,3)*River%min_rate*(1.047**((min(303.15,(max(270.15,temperature(i,j)))))-293.15))
                    River%storage_c(i,j,3) = River%storage_c(i,j,3) - River%removal_c(i,j,3)* River%dt_slow
		    
		    River%removal_c(i,j,4) = River%storage_c(i,j,4)*River%nit_rate*(1.047**((min(303.15,(max(270.15,temperature(i,j)))))-293.15)) &
		                           - River%storage_c(i,j,3)*River%min_rate*(1.047**((min(303.15,(max(270.15,temperature(i,j)))))-293.15))
                    River%storage_c(i,j,4) = River%storage_c(i,j,4) - River%removal_c(i,j,4)* River%dt_slow
		    
		    River%removal_c(i,j,5) = River%storage_c(i,j,5)*(min(117.0,max(0.11, &
		                             (0.5588 * ((max(1e-30,River%storage_c(i,j,5))*1e9 / (max(1e-30,River%storage(i,j))*14.01))**(-0.478)) * &
		                             (max(1e-30,River%depth(i,j))**(-0.612)) * 1.90))) / 86400)* &
					     (1.047**((min(303.15,(max(270.15,temperature(i,j)))))-293.15)) &
		                           - River%storage_c(i,j,4)*River%nit_rate*(1.047**((min(303.15,(max(270.15,temperature(i,j)))))-293.15))
                    River%storage_c(i,j,5) = River%storage_c(i,j,5) - River%removal_c(i,j,5)* River%dt_slow
