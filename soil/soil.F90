...
character(len=*), parameter, private   :: &
    module_name = 'soil',&
    version     = '$Id: soil.F90,v 17.0.6.6 2010/08/23 20:09:56 slm Exp $',&
...
! apply boundary flows to soil water and move soil water vertically.
  ...
  real, intent(in) :: &
     ...
     subs_evap,  &
     sorg_depth ! depth of organic soil, m, for drainage and uptake aggregation
  logical, intent(in) :: use_tfreeze_in_grnd_latent
  real, intent(out) :: &
     soil_levap, soil_fevap, soil_melt, &
     soil_lrunf, soil_hlrunf, soil_Ttop, soil_Ctop, &
     sorg_uptake, & ! uptake aggregated over the organic soil depth, kg/(m2 s)
     sorg_drain, &  ! export of water from the organic soil layer, kg/(m2 s)
  ...
  real :: &
       ...
       K_r, & ! root membrane permeability, kg/(m3 s)
       r_r, & ! root radius, m
  ...
  real :: flow_sorg ! water flow at the bottom of organic soil, kg /(m2 time_step)
  ...
  case ( UPTAKE_DARCY2D, UPTAKE_DARCY2D_LIN )     
     ! for Darcy-flow uptake, find the root water potential to satisfy actual
  ...
  ! calculate total positive uptake from organic soil
  sorg_uptake = soil_average(max(uptake(:)/dz(:),0.0),sorg_depth,WGHT_UNIFORM)*sorg_depth
...
ENDIF                                              ! BYPASS_RICHARDS_WHEN_STIFF
  ...
  ! calculate total drainage from top organic layer of the soil.
  ! first contribution is from 2D divergence of the water
  sorg_drain = soil_average(max(div(:),0.0)/dz(:),sorg_depth,WGHT_UNIFORM)*sorg_depth
  ! find the deepest layer that overlaps organic soil
  do l = 1,num_l
     if (sorg_depth<=zhalf(l+1)) exit
  enddo
  ! compute flow at the bottom of the organic soil. Assume that the
  ! divergence of vertical flow is uniform across the layer, therefore 
  ! flow is linear with z.
  flow_sorg = flow(l) + (flow(l+1)-flow(l))*(min(sorg_depth,zhalf(l+1))-zhalf(l)) &
                                           /(zhalf(l+1)-zhalf(l))
  ! do not take it into account if the flow is negative (upwards): it does
  ! not contribute to the drainage in this case
  flow_sorg = max(flow_sorg, 0.0)
  ! add flow component
  sorg_drain = sorg_drain + flow_sorg/delta_time
  if(is_watch_point()) then
     __DEBUG2__(sorg_depth,sorg_drain)
  endif
...
!    enddo
  ...
  ! calculate total surface runoff for nitrogen calculations
  surf_runoff = lrunf_sn+lrunf_ie
