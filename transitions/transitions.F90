...
character(len=*), parameter   :: &
     version = '$Id: transitions.F90,v 18.0.2.3 2010/07/30 00:50:09 slm Exp $', &
...
subroutine land_transitions_0d(d_list,d_kinds,a_kinds,area)
  ...
  ! cell totals
  lmass0 = 0 ; fmass0 = 0 ; cmass0 = 0 ; nmass0 = 0 ; heat0 = 0
  ...
  do while (ts /= te)
     ...
     nmass0 = nmass0 + land_tile_nitrogen(ptr)*ptr%frac
  ...
  lmass1 = 0 ; fmass1 = 0 ; cmass1 = 0 ; nmass1 = 0 ; heat1 = 0
  ...
  do while (ts /= te)
     ...
     nmass1 = nmass1 + land_tile_nitrogen(ptr)*ptr%frac
  ...
  enddo
  call check_conservation ('land_transitions_0d', 'liquid water', lmass0, lmass1, 1e-6, &
     lnd%time, conservation_opt)
  call check_conservation ('land_transitions_0d', 'frozen water', fmass0, fmass1, 1e-6, &
     lnd%time, conservation_opt)
  !call check_conservation ('land_transitions_0d', 'carbon'      , cmass0, cmass1, 1e-6, &
  !   lnd%time, conservation_opt)
  !call check_conservation ('land_transitions_0d', 'nitrogen'    , nmass0, nmass1, 1e-6, &
  !   lnd%time, conservation_opt)
  call check_conservation ('land_transitions_0d', 'canopy air heat content', &
     cana_heat0 , cana_heat1 , 1e-6, lnd%time, conservation_opt)
  call check_conservation ('land_transitions_0d', 'vegetation heat content', &
     vegn_heat0 , vegn_heat1 , 1e-6, lnd%time, conservation_opt)
  call check_conservation ('land_transitions_0d', 'snow heat content',       &
     snow_heat0 , snow_heat1 , 1e-6, lnd%time, conservation_opt)
  call check_conservation ('land_transitions_0d', 'soil heat content',       &
     soil_heat0 , soil_heat1 , 1e-6, lnd%time, conservation_opt)
  call check_conservation ('land_transitions_0d', 'heat content', heat0 , heat1 , 1e-6, &
     lnd%time, conservation_opt)
...
subroutine split_changing_tile_parts(d_list,d_kind,a_kind,dfrac,a_list)
  ...
  if(overshoot_opt > 0 .and.dfrac>area+overshoot_tolerance) then
