...
use fms_mod, only: write_version_number, error_mesg, NOTE,FATAL, file_exist, close_file, &
     open_namelist_file, check_nml_error, stdlog, stdout
...
use land_substances_mod, only : n_substances, inq_substance
use land_io_mod, only : read_field
use land_tile_io_mod, only : &
     create_tile_out_file, &
     write_tile_data_r0d_fptr, write_tile_data_i0d_fptr, &
     write_tile_data_r1d_fptr, write_tile_data_r2d_fptr, &
     read_tile_data_r0d_fptr,  read_tile_data_i0d_fptr,  &
     read_tile_data_r1d_fptr,  read_tile_data_r2d_fptr,  &
...
use vegn_data_mod, only : SP_C4GRASS, LEAF_ON, LU_NTRL, read_vegn_data_namelist, &
     ...
     spdata, N_HARV_POOLS, HARV_POOL_NAMES, &
     CMPT_LEAF, CMPT_ROOT, CMPT_SAPWOOD, CMPT_WOOD, isub_C
...
use vegn_dynamics_mod, only : vegn_dynamics_init, vegn_carbon_int, vegn_growth, &
     vegn_daily_npp, vegn_phenology, vegn_biogeography, soil_carbon_int, &
     update_soil_pools
...
use nitrogen_data_mod, only: do_nitrogen, init_nfix_per_bliv, isub_N
...
character(len=*), private, parameter :: &
   version = '$Id: vegetation.F90,v 18.0.2.24.2.3.2.2 2011/10/04 23:06:33 Sergey.Malyshev Exp $', &
   tagname = '$Name:  $', &
   module_name = 'vegn'
...
integer :: id_vegn_type, id_temp, id_wl, id_ws, id_height, id_lai, id_sai, id_leaf_size, &
   ...
   id_nl, id_nlv, id_nr, id_nsw, id_nwood, id_nstore, id_nstore_opt, id_ntot, &
   id_fln, id_sln, id_ssn, id_rsn, id_fln_in, id_sln_in, id_fln_in_rate, id_sln_in_rate, & 
   id_con_v_h, id_con_v_v, id_fuel, &
   id_fsc_in, id_fsc_out, id_ssc_in, &
   ...
   id_phot_co2, id_ngain, id_nwoodgain, id_nsf, id_pl, id_pr, id_psw, id_psw_alphasw
integer, allocatable :: id_harv_pool(:,:), id_harv_rate(:,:), &
   id_t_harv_pool(:), id_t_harv_rate(:), id_csmoke_pool(:), id_csmoke_rate(:)
...
subroutine vegn_init ( id_lon, id_lat, id_band )
  ...
  integer :: isub ! substance iterator
  character(16) :: name ! name of the substance
  character(NF_MAX_NAME) :: varname ! name of various netcdf variables
  ...
  dt_slow_yr = time_type_to_real(lnd%dt_fast)/seconds_per_year
  ...
  if (restart_1_exists) then
     ...
     ! burned carbon pool and rate. Note that transition from no-substances
     ! restarts is handled somewhat differently for csmoke. This is because there
     ! are intermediate restarts, where csmoke pools/rates are only for carbon,
     ! but the other variables are substance-specific.
     if(nfu_inq_var(unit,'csmoke_pool')==NF_NOERR) &
       call read_tile_data_r1d_fptr(unit,'csmoke_pool',vegn_csmoke_pool_ptr,isub_C)
     if(nfu_inq_var(unit,'csmoke_rate')==NF_NOERR) &
       call read_tile_data_r1d_fptr(unit,'csmoke_rate',vegn_csmoke_rate_ptr,isub_C)

     ! figure out what kind of restart we have: old one [prior to introduction
     ! of substances], or newer kind, with substance differentiation
     if (nfu_inq_var(unit,'fsc_pool')==NF_NOERR) then
       ! read an older restart, created prior to introduction of substances --
       ! only pools and rates for carbon exist
       call read_tile_data_r1d_fptr(unit,'fsc_pool',vegn_fsc_pool_ptr,isub_C)
       call read_tile_data_r1d_fptr(unit,'fsc_rate',vegn_fsc_pool_ptr,isub_C)
       call read_tile_data_r1d_fptr(unit,'ssc_pool',vegn_fsc_pool_ptr,isub_C)
       call read_tile_data_r1d_fptr(unit,'ssc_rate',vegn_fsc_pool_ptr,isub_C)
       ! harvesting pools and rates
       do i = 1, N_HARV_POOLS
         if (nfu_inq_var(unit,trim(HARV_POOL_NAMES(i))//'_harv_pool')==NF_NOERR) then
           call read_tile_data_r2d_fptr(unit,trim(HARV_POOL_NAMES(i))//'_harv_pool', &
                                        vegn_harv_pool_ptr,i,isub_C)
         endif
         if (nfu_inq_var(unit,trim(HARV_POOL_NAMES(i))//'_harv_rate')==NF_NOERR) then
           call read_tile_data_r2d_fptr(unit,trim(HARV_POOL_NAMES(i))//'_harv_rate', &
                                        vegn_harv_rate_ptr,i,isub_C)
         endif
       enddo
       ! the pools and rates for the substances other than carbon are set to 
       ! zero when the tiles are created
     else
       ! read restart with pools/rates differentiated by substance
       do isub = 1,n_substances
         call inq_substance(isub,name)
         call read_tile_data_r1d_fptr(unit,trim(name)//'_fsc_pool',vegn_fsc_pool_ptr,isub)
         call read_tile_data_r1d_fptr(unit,trim(name)//'_fsc_rate',vegn_fsc_rate_ptr,isub)
         call read_tile_data_r1d_fptr(unit,trim(name)//'_ssc_pool',vegn_ssc_pool_ptr,isub)
         call read_tile_data_r1d_fptr(unit,trim(name)//'_ssc_rate',vegn_ssc_rate_ptr,isub)
         ! harvesting pools and rates
         do i = 1, N_HARV_POOLS
           varname = trim(name)//'_'//trim(HARV_POOL_NAMES(i))//'_harv_pool'
           if (nfu_inq_var(unit,varname)==NF_NOERR) &
               call read_tile_data_r2d_fptr(unit,varname,vegn_harv_pool_ptr,i,isub)
           varname = trim(name)//'_'//trim(HARV_POOL_NAMES(i))//'_harv_rate'
           if (nfu_inq_var(unit,varname)==NF_NOERR) &
              call read_tile_data_r2d_fptr(unit,varname,vegn_harv_rate_ptr,i,isub)
         enddo
         varname = trim(name)//'_smoke_pool'
         if(nfu_inq_var(unit,varname)==NF_NOERR) &
           call read_tile_data_r1d_fptr(unit,varname,vegn_csmoke_pool_ptr,isub)
         varname = trim(name)//'_smoke_rate'
         if(nfu_inq_var(unit,varname)==NF_NOERR) &
           call read_tile_data_r1d_fptr(unit,varname,vegn_csmoke_rate_ptr,isub)
       enddo
     endif
     ...
     ! read nitrogen-related variables
     if (do_nitrogen) then
        ! fast litter pool
        call read_tile_data_r0d_fptr(unit,'C_fl', vegn_fast_litter_C_ptr)
        call read_tile_data_r0d_fptr(unit,'N_fl', vegn_fast_litter_N_ptr)
        call read_tile_data_r0d_fptr(unit,'fsol_fl', vegn_fast_litter_fsol_ptr)
        ! slow litter pool
        call read_tile_data_r0d_fptr(unit,'C_sl', vegn_slow_litter_C_ptr)
        call read_tile_data_r0d_fptr(unit,'N_sl', vegn_slow_litter_N_ptr)
        call read_tile_data_r0d_fptr(unit,'fsol_sl', vegn_slow_litter_fsol_ptr)
        ! slow soil pool
        ! slow soil carbon read elsewhere
        call read_tile_data_r0d_fptr(unit,'N_ss', vegn_slow_soil_N_ptr)
        call read_tile_data_r0d_fptr(unit,'fsol_ss', vegn_slow_soil_fsol_ptr)
        ! passive soil pool
        call read_tile_data_r0d_fptr(unit,'C_rs', vegn_res_soil_C_ptr)
        call read_tile_data_r0d_fptr(unit,'N_rs', vegn_res_soil_N_ptr)
        ! mineralized nitrogen pools
        call read_tile_data_r0d_fptr(unit,'amm', vegn_amm_ptr)
        call read_tile_data_r0d_fptr(unit,'nitr', vegn_nitr_ptr)
        ! cohort-level nitrogen stores
        call read_cohort_data_r0d_fptr(unit,'N_l', cohort_nl_ptr)
        call read_cohort_data_r0d_fptr(unit,'N_r', cohort_nr_ptr)
        call read_cohort_data_r0d_fptr(unit,'N_lv', cohort_nlv_ptr)
        call read_cohort_data_r0d_fptr(unit,'N_sw', cohort_nsw_ptr)
        call read_cohort_data_r0d_fptr(unit,'N_wood', cohort_nwood_ptr)
        call read_cohort_data_r0d_fptr(unit,'N_store', cohort_nstore_ptr)
        call read_cohort_data_r0d_fptr(unit,'N_living', cohort_nliving_ptr)
        ! optimum nitrogen storage
        if(nfu_inq_var(unit,'N_store_opt')==NF_NOERR) &
           call read_cohort_data_r0d_fptr(unit,'N_store_opt', cohort_nstore_opt_ptr)
        ! nitrogen fixation status     
        call read_cohort_data_r0d_fptr(unit,'nfix_per_bliv', cohort_nfix_per_bliv_ptr)
        if(nfu_inq_var(unit,'nfixrate')==NF_NOERR) &
           call read_cohort_data_r0d_fptr(unit,'nfixrate', cohort_nfixrate_ptr)
     endif
  ...
  do while(ce /= te)
     ...
     ! cold-start initialize nitrogen-related variables
     cohort%nfix_per_bliv = init_nfix_per_bliv
     sp = cohort%species
     cohort%nl    = cohort%bl    / spdata(sp)%CtoN(CMPT_LEAF)
     ! cohort%nlv   = cohort%blv   / spdata(sp)%CtoN(CMPT_VLEAF)
     cohort%nr    = cohort%br    / spdata(sp)%CtoN(CMPT_ROOT)
     cohort%nsw   = cohort%bsw   / spdata(sp)%CtoN(CMPT_SAPWOOD)
     cohort%nwood = cohort%bwood / spdata(sp)%CtoN(CMPT_WOOD)
     cohort%nliving = cohort%nl+cohort%nr+cohort%nlv+cohort%nsw
     cohort%nstore_opt = cohort%bliving/35.0
  enddo
  ...
  if (restart_1_exists) then
     ...
     if (do_nitrogen) then
       call read_tile_data_r0d_fptr(unit,'slc_in_eq',vegn_slc_in_eq_ptr)
       call read_tile_data_r0d_fptr(unit,'ssc_in_eq',vegn_ssc_in_eq_ptr)
       call read_tile_data_r0d_fptr(unit,'resc_in_eq',vegn_resc_in_eq_ptr)
       call read_tile_data_r0d_fptr(unit,'sln_in_eq',vegn_sln_in_eq_ptr)
       call read_tile_data_r0d_fptr(unit,'A_eq',vegn_A_eq_ptr)
       call read_tile_data_r0d_fptr(unit,'k_ls_eq',vegn_k_ls_eq_ptr)
       call read_tile_data_r0d_fptr(unit,'k_ss_eq',vegn_k_ss_eq_ptr)
     endif
     __NF_ASRT__(nf_close(unit))     
  endif
 ...
 id_pl   = register_tiled_diag_field (module_name, 'pl', (/id_lon,id_lat/), &
      time, 'fraction of biomass allocated to leaves', '1', missing_value=-100.0)
 id_pr   = register_tiled_diag_field (module_name, 'pr', (/id_lon,id_lat/), &
      time, 'fraction of biomass allocated to roots', '1', missing_value=-100.0)
 id_psw   = register_tiled_diag_field (module_name, 'psw', (/id_lon,id_lat/), &
      time, 'fraction of biomass allocated to sapwood', '1', missing_value=-100.0)
 id_psw_alphasw  = register_tiled_diag_field (module_name, 'psw_alphasw', (/id_lon,id_lat/), &
      time, 'fraction of biomass allocated to sapwood times rate of sapwood-to-wood &
      transformation', '1', missing_value=-100.0)
...
subroutine vegn_diag_init ( id_lon, id_lat, id_band, time )
  ...
  integer :: isub ! substance iterator
  character( 16) :: name  ! name of the substance
  character(128) :: lname ! long name of the substance

  allocate(id_harv_pool(N_HARV_POOLS,n_substances)); id_harv_pool = -1
  allocate(id_harv_rate(N_HARV_POOLS,n_substances)); id_harv_rate = -1
  allocate(id_t_harv_pool(n_substances)) ; id_t_harv_pool = -1
  allocate(id_t_harv_rate(n_substances)) ; id_t_harv_rate = -1
  allocate(id_csmoke_pool(n_substances)) ; id_csmoke_pool = -1
  allocate(id_csmoke_rate(n_substances)) ; id_csmoke_rate = -1
  ...
  id_nl = register_tiled_diag_field ( module_name, 'nl',  &
       (/id_lon,id_lat/), time, 'nitrogen in leaves', 'kg N/m2', missing_value=-1.0 )
  id_nlv = register_tiled_diag_field ( module_name, 'nlv',  &
       (/id_lon,id_lat/), time, 'nitrogen in labile store', 'kg N/m2', missing_value=-1.0 )
  id_nr = register_tiled_diag_field ( module_name, 'nr',  &
       (/id_lon,id_lat/), time, 'nitrogen in fine roots', 'kg N/m2', missing_value=-1.0 )
  id_nsw = register_tiled_diag_field ( module_name, 'nsw',  &
       (/id_lon,id_lat/), time, 'nitrogen in sapwood', 'kg N/m2', missing_value=-1.0 )
  id_nwood = register_tiled_diag_field ( module_name, 'nwood',  &
       (/id_lon,id_lat/), time, 'nitrogen in heartwood', 'kg N/m2', missing_value=-1.0 )
  id_nstore = register_tiled_diag_field ( module_name, 'nstore',  &
       (/id_lon,id_lat/), time, 'nitrogen in storage', 'kg N/m2', missing_value=-1.0 )
  id_nstore_opt = register_tiled_diag_field ( module_name, 'nstore_opt', &
       (/id_lon,id_lat/), time, 'optimal nstore', 'kg N/m2', missing_value=-1.0)
  id_fln = register_tiled_diag_field ( module_name, 'fln',  &
       (/id_lon,id_lat/), time, 'nitrogen in fast litter', 'kg N/m2', missing_value=-1.0 )
  id_sln = register_tiled_diag_field ( module_name, 'sln',  &
       (/id_lon,id_lat/), time, 'nitrogen in slow litter', 'kg N/m2', missing_value=-1.0 )
  id_ssn = register_tiled_diag_field ( module_name, 'ssn',  &
       (/id_lon,id_lat/), time, 'nitrogen in slow soil', 'kg N/m2', missing_value=-1.0 )
  id_rsn = register_tiled_diag_field ( module_name, 'rsn',  &
       (/id_lon,id_lat/), time, 'nitrogen in passive (resilient) soil', 'kg N/m2', &
       missing_value=-1.0 )

  id_ngain = register_tiled_diag_field ( module_name, 'ngain', (/id_lon,id_lat/), &
       time, 'plant nitrogen uptake accumulator', 'kg N/m2', missing_value=-999.0)
  id_nwoodgain = register_tiled_diag_field ( module_name, 'nwoodgain', (/id_lon,id_lat/), &
       time, 'wood nitrogen uptake accumulator', 'kg N/m2', missing_value=-999.0)
  id_ntot = register_tiled_diag_field ( module_name, 'ntotal',  &
       (/id_lon,id_lat/), time, 'total nitrogen', 'kg N/m2', missing_value=-1.0 )
  id_fln_in = register_tiled_diag_field ( module_name, 'fln_in', (/id_lon,id_lat/), &
       time, 'nitrogen input to fast litter accumulator', 'kg N/m2', missing_value=-999.0)
  id_sln_in = register_tiled_diag_field ( module_name, 'sln_in', (/id_lon,id_lat/), &
       time, 'nitrogen input to slow litter accumulator', 'kg N/m2', missing_value=-999.0)
  id_fln_in_rate = register_tiled_diag_field ( module_name, 'fln_in_rate', (/id_lon,id_lat/), &
       time, 'rate of nitrogen input to fast litter', 'kg N/(m2 year)', missing_value=-999.0)
  id_sln_in_rate = register_tiled_diag_field ( module_name, 'sln_in_rate', (/id_lon,id_lat/), &
       time, 'rate of nitrogen input to slow litter', 'kg N/(m2 year)', missing_value=-999.0)
  ...
  do isub = 1,n_substances
    ! get the substance information
    call inq_substance(isub,name=name,long_name=lname)
    ! register total harvested pool and rate information
    id_t_harv_pool(isub) = register_tiled_diag_field( module_name, &
       trim(name)//'_harv_pool', (/id_lon,id_lat/), time, &
       'total accumulated amount of harvested '//trim(lname), 'kg '//trim(name)//'/m2', &
       missing_value=-999.0)
    id_t_harv_rate(isub) = register_tiled_diag_field( module_name, &
       trim(name)//'_harv_rate', (/id_lon,id_lat/), time, &
       'total rate of release of harvested '//trim(lname)//' to the atmosphere', &
       'kg '//trim(name)//'/(m2 year)', missing_value=-999.0)
    do i = 1,N_HARV_POOLS
      id_harv_pool(i,isub) = register_tiled_diag_field( module_name, &
          trim(name)//'_'//trim(HARV_POOL_NAMES(i))//'_harv_pool', (/id_lon,id_lat/), &
          time, 'accumulated amount of harvested '//trim(lname), 'kg '//trim(name)//'/m2', &
          missing_value=-999.0)
      id_harv_rate(i,isub) = register_tiled_diag_field( module_name, &
          trim(name)//'_'//trim(HARV_POOL_NAMES(i))//'_harv_rate', &
          (/id_lon,id_lat/), time, &
          'rate of release of harvested '//trim(lname)//' to the atmosphere', &
          'kg '//trim(name)//'/(m2 year)', missing_value=-999.0)
    enddo
    id_csmoke_pool(isub) = register_tiled_diag_field( module_name, &
       trim(name)//'_smoke_pool', (/id_lon,id_lat/), time, &
       trim(lname)//' lost to fire', 'kg '//trim(name)//'/m2', &
       missing_value=-999.0)
    id_csmoke_rate(isub) = register_tiled_diag_field( module_name, &
       trim(name)//'_smoke_rate', (/id_lon,id_lat/), time, &
       'rate of release of '//trim(lname)//' lost through fire to the atmosphere', &
       'kg '//trim(name)//'/m2', &
       missing_value=-999.0)
  enddo
  
  ! add the diag field aliases to preserve compatibility with older code output
  call add_tiled_diag_field_alias( id_t_harv_pool(isub_C), &
        module_name, 'harv_pool', (/id_lon,id_lat/), &
       time, 'total accumualted amount of harvested carbon', 'kg C/m2', missing_value=-999.0)
  call add_tiled_diag_field_alias( id_t_harv_rate(isub_C), &
       module_name, 'harv_rate', (/id_lon,id_lat/), &
       time, 'total rate of release of harvested carbon to the atmosphere', &
       'kg C/(m2 year)', missing_value=-999.0)
  do i = 1,N_HARV_POOLS
     call add_tiled_diag_field_alias( id_harv_pool(i,isub_C), &
          module_name, &
          trim(HARV_POOL_NAMES(i))//'_harv_pool', (/id_lon,id_lat/), time, &
          'accumulated amount of harvested carbon', 'kg C/m2', missing_value=-999.0)
     call add_tiled_diag_field_alias( id_harv_rate(i,isub_C), &
          module_name, &
          trim(HARV_POOL_NAMES(i))//'_harv_rate', (/id_lon,id_lat/), time, &
          'rate of release of harvested carbon to the atmosphere', 'kg C/(m2 year)', &
          missing_value=-999.0)
  enddo
  call add_tiled_diag_field_alias( id_csmoke_pool(isub_C), &
       module_name, 'csmoke', (/id_lon, id_lat/), &
       time, 'carbon lost through fire', 'kg C/m2', missing_value=-999.0)
  call add_tiled_diag_field_alias( id_csmoke_rate(isub_C), &
       module_name, 'csmoke_rate', (/id_lon, id_lat/), &
  ...
  id_fsc_rate = register_tiled_diag_field (module_name, 'fsc_rate', (/id_lon, id_lat/), &
       time, 'rate of conversion of fsc_pool to the fast soil_carbon', 'kg C/(m2 year)', &
       missing_value=-999.0)
  ...
  id_ssc_rate = register_tiled_diag_field (module_name, 'ssc_rate', (/id_lon, id_lat/), &
       time, 'rate of conversion of ssc_pool to the fast soil_carbon', 'kg C/(m2 year)', &
       missing_value=-999.0)
  ...
  id_phot_co2 = register_tiled_diag_field (module_name, 'qco2_phot',(/id_lon,id_lat/), &
       ...
       missing_value=-1.0)
  id_nsf   = register_tiled_diag_field (module_name, 'nsf', (/id_lon,id_lat/), &
       time, 'fraction of nstore to max possible', 'kg/kg', missing_value=-100.0)
end subroutine
...
!     call write_cohort_data_r0d_fptr(unit,'intercept_l', cohort_cmc_ptr, 'intercepted water per cohort','kg/m2')
  ...
  call write_tile_data_r0d_fptr(unit,'age',vegn_age_ptr,'vegetation age', 'year')
  ...
  do isub = 1,n_substances
    call inq_substance(isub,name=name,long_name=lname)
    ! intermediate pools and rates
    call write_tile_data_r1d_fptr(unit,trim(name)//'_fsc_pool', vegn_fsc_pool_ptr, isub, &
         'intermediate pool for fast soil '//trim(lname)//' input', 'kg '//trim(name)//'/m2')
    call write_tile_data_r1d_fptr(unit,trim(name)//'_fsc_rate', vegn_fsc_rate_ptr, isub, &
         'conversion rate of fsc_pool to fast soil '//trim(lname), 'kg '//trim(name)//'/(m2 year)')
    call write_tile_data_r1d_fptr(unit,trim(name)//'_ssc_pool', vegn_ssc_pool_ptr, isub, &
         'intermediate pool for slow soil '//trim(lname)//' input', 'kg '//trim(name)//'/m2')
    call write_tile_data_r1d_fptr(unit,trim(name)//'_ssc_rate', vegn_ssc_rate_ptr, isub, &
         'conversion rate of ssc_pool to slow soil '//trim(lname), 'kg '//trim(name)//'/(m2 year)')
    ! harvesting pools and rates
    do i = 1, N_HARV_POOLS
       call write_tile_data_r2d_fptr(unit, &
            trim(name)//'_'//trim(HARV_POOL_NAMES(i))//'_harv_pool', &
            vegn_harv_pool_ptr, i, isub, 'accumulated harvested '//trim(lname),&
            'kg '//trim(name)//'/m2')
       call write_tile_data_r2d_fptr(unit, &
            trim(name)//'_'//trim(HARV_POOL_NAMES(i))//'_harv_rate', &
            vegn_harv_rate_ptr, i, isub, 'rate of release of harvested '//trim(lname)//&
               ' to the atmosphere','kg '//trim(name)//'/(m2 year)')
    enddo
    ! burned carbon pool and rate
    call write_tile_data_r1d_fptr(unit,trim(name)//'_smoke_pool',vegn_csmoke_pool_ptr,isub,&
         trim(lname)//' lost through fires', 'kg '//trim(name)//'/m2')
    call write_tile_data_r1d_fptr(unit,trim(name)//'_smoke_rate',vegn_csmoke_rate_ptr,isub,&
         'rate of release of '//trim(lname)//' lost through fires to the atmosphere', &
         'kg '//trim(name)//'/(m2 year)')
  enddo

  ! write nitrogen-related variables
  if (do_nitrogen) then
     ! fast litter pool
     call write_tile_data_r0d_fptr(unit,'C_fl', vegn_fast_litter_C_ptr, 'fast litter carbon', 'kg C/m2')
     call write_tile_data_r0d_fptr(unit,'N_fl', vegn_fast_litter_N_ptr, 'fast litter nitrogen', 'kg N/m2')
     call write_tile_data_r0d_fptr(unit,'fsol_fl', vegn_fast_litter_fsol_ptr, 'soluble fraction of fast litter nitrogen')
     ! slow litter pool
     call write_tile_data_r0d_fptr(unit,'C_sl', vegn_slow_litter_C_ptr, 'slow litter carbon', 'kg C/m2')
     call write_tile_data_r0d_fptr(unit,'N_sl', vegn_slow_litter_N_ptr, 'slow litter nitrogen', 'kg N/m2')
     call write_tile_data_r0d_fptr(unit,'fsol_sl', vegn_slow_litter_fsol_ptr, 'soluble fraction of slow litter nitrogen')
     ! slow soil pool
     ! slow soil carbon is written elsewhere
     call write_tile_data_r0d_fptr(unit,'N_ss', vegn_slow_soil_N_ptr, 'slow soil nitrogen', 'kg N/m2')
     call write_tile_data_r0d_fptr(unit,'fsol_ss', vegn_slow_soil_fsol_ptr, 'soluble fraction of slow soil nitrogen')
     ! passive soil pool
     call write_tile_data_r0d_fptr(unit,'C_rs', vegn_res_soil_C_ptr, 'passive (resilient) soil carbon', 'kg C/m2')
     call write_tile_data_r0d_fptr(unit,'N_rs', vegn_res_soil_N_ptr, 'passive (resilient) soil nitrogen', 'kg N/m2')
     ! mineralized nitrogen pools
     call write_tile_data_r0d_fptr(unit,'amm', vegn_amm_ptr, 'soil ammonium', 'kg N/m2')
     call write_tile_data_r0d_fptr(unit,'nitr', vegn_nitr_ptr, 'soil nitrate', 'kg N/m2')
     ! cohort-level nitrogen stores
     call write_cohort_data_r0d_fptr(unit,'N_l', cohort_nl_ptr, 'nitrogen in leaves','kg N/m2')
     call write_cohort_data_r0d_fptr(unit,'N_r', cohort_nr_ptr, 'nitrogen in roots','kg N/m2')
     call write_cohort_data_r0d_fptr(unit,'N_lv', cohort_nlv_ptr, 'nitrogen in labile storage','kg N/m2')
     call write_cohort_data_r0d_fptr(unit,'N_sw', cohort_nsw_ptr, 'nitrogen in sapwood','kg N/m2')
     call write_cohort_data_r0d_fptr(unit,'N_wood', cohort_nwood_ptr, 'nitrogen in heartwood','kg N/m2')
     call write_cohort_data_r0d_fptr(unit,'N_store', cohort_nstore_ptr, 'nitrogen in storage','kg N/m2')
     call write_cohort_data_r0d_fptr(unit,'N_living', cohort_nliving_ptr, 'total nitrogen in living tissues','kg N/m2')
     ! optimum storage
     call write_cohort_data_r0d_fptr(unit,'N_store_opt', cohort_nstore_opt_ptr, 'optimum amount of storage nitrogen','kg N/m2')
     ! nitrogen fixation status
     call write_cohort_data_r0d_fptr(unit,'nfix_per_bliv', cohort_nfix_per_bliv_ptr, &
            'nitrogen fixation rate per labile carbon mass','kg N/(kg C year)')
     ! nfixrate needs to be stored for reproducibility in comp_nfix_2 case, since
     ! it is not calculated from nfix_per_bliv for this option
     call write_cohort_data_r0d_fptr(unit,'nfixrate', cohort_nfixrate_ptr, &
            'nitrogen fixation rate','kg N/(kg C year)')
  endif
  ...
  if (write_soil_carbon_restart) then
     ...
     if (do_nitrogen) then
       call write_tile_data_r0d_fptr(unit,'slc_in_eq',vegn_slc_in_eq_ptr,'slow litter carbon input for equilibrium', 'kg/m2')
       call write_tile_data_r0d_fptr(unit,'ssc_in_eq',vegn_ssc_in_eq_ptr,'equilibrium carbon input in slow soil', 'kg/m2')
       call write_tile_data_r0d_fptr(unit,'resc_in_eq',vegn_resc_in_eq_ptr,'equilibrium carbon input in passive soil', 'kg/m2')
       call write_tile_data_r0d_fptr(unit,'sln_in_eq',vegn_sln_in_eq_ptr,'slow litter nitrogen input for equilibrium', 'kg/m2')
       call write_tile_data_r0d_fptr(unit,'A_eq',vegn_A_eq_ptr, 'A function equilibrium')
       call write_tile_data_r0d_fptr(unit,'k_ls_eq',vegn_k_ls_eq_ptr,'decomp rate of slow litter', '1/year')
       call write_tile_data_r0d_fptr(unit,'k_ss_eq',vegn_k_ss_eq_ptr,'decomp rate of slow soil', '1/year')
       ! are those necessary for equilibration?
!        call write_tile_data(unit,'slc_eq',get_patch_slc_eq(tiles%patch),&
!             'equilibrium carbon in slow litter', 'kg/m2')
!        call write_tile_data(unit,'ssc_eq',get_patch_ssc_eq(tiles%patch),&
!             'equilibrium carbon in slow soil', 'kg/m2')
!        call write_tile_data(unit,'resc_eq',get_patch_resc_eq(tiles%patch),&
!             'equilibrium carbon in passive soil', 'kg/m2')
!
!        call write_tile_data(unit,'sln_eq',get_patch_sln_eq(tiles%patch),&
!             'equilibrium nitrogen in slow litter', 'kgN/m2')
!        call write_tile_data(unit,'ssn_eq',get_patch_ssn_eq(tiles%patch),&
!             'equilibrium nitrogen in slow soil', 'kg/m2')
!        call write_tile_data(unit,'resn_eq',get_patch_resn_eq(tiles%patch),&
!             'equilibrium nitrogen in passive soil', 'kg/m2')
     endif
     __NF_ASRT__(nf_close(unit))
  endif
...
subroutine vegn_step_1 ( vegn, diag, &
  ...
  call send_tile_data(id_nsf, cohort%nsf, diag)
...
subroutine vegn_step_3(vegn, soil, cana_T, precip, ndep_nit, ndep_amm, ndep_don, nfert, nfm_nit, nfm_amm, nfm_don, &
                       sorg_depth, sorg_uptake, sorg_drain, surf_runoff, &
                       vegn_fco2, donleach, ammleach, nitrleach, diag)
  type(vegn_tile_type), intent(inout) :: vegn
  type(soil_tile_type), intent(in)    :: soil
  real, intent(in) :: cana_T ! canopy temperature, deg K
  real, intent(in) :: precip ! total (rain+snow) precipitation, kg/(m2 s)
  real, intent(in) :: ndep_nit, ndep_amm, ndep_don ! nitrate, ammonium, and DON deposition, kg N/(m2 yr)
  real, intent(in) :: nfert, nfm_nit, nfm_amm, nfm_don  ! nitrogen fertilization, kg N/(m2 yr)
  real, intent(in) :: sorg_depth  ! depth of organic soil for soil nitrogen calculations, m
  real, intent(in) :: sorg_uptake ! uptake from organic soil, kg/(m2 s)
  real, intent(in) :: sorg_drain  ! drainage of water from organic soil, kg/(m2 s)
  real, intent(in) :: surf_runoff ! surface runoff, kg/(m2 s)
  real, intent(out) :: vegn_fco2  ! co2 flux from vegetation, kg CO2/(m2 s)
  real, intent(out) :: donleach   ! leaching of dissolved organic nitrogen, kg N/(m2 s)
  real, intent(out) :: ammleach   ! leaching of ammonium, kg N/(m2 s)
  real, intent(out) :: nitrleach  ! leaching of nitrate, kg N/(m2 s)
  type(diag_buff_type), intent(inout) :: diag
  ...
  if (do_nitrogen) then
     tsoil = soil_ave_temp (soil,sorg_depth,WGHT_UNIFORM)
     theta = soil_ave_theta(soil,sorg_depth,WGHT_UNIFORM)
  else
     tsoil = soil_ave_temp (soil,soil_carbon_depth_scale,WGHT_EXPONENTIAL)
     theta = soil_ave_theta(soil,depth_ave,WGHT_EXPONENTIAL)
  endif
  ...
  call vegn_carbon_int(vegn, tsoil, diag)
  ! TODO:
  ! (1) replace precipitation with nitrogen deposition
  call soil_carbon_int(vegn, sorg_depth, tsoil, theta, &
        ndep_nit, ndep_amm, ndep_don, nfert, nfm_nit, nfm_amm, nfm_don, &
        sorg_uptake, sorg_drain, surf_runoff, diag)
  donleach=vegn%donleach
  ammleach=vegn%ammleach
  nitrleach=vegn%nitrleach

  ! calculate net ecosystem production
  vegn%nep = vegn%npp - vegn%rh

  ! transfer the carbon and nitrogen from the intermediate pools to the soil
  ...
  vegn%harv_pool    = vegn%harv_pool - vegn%harv_rate*dt_fast_yr
  ! --- calculate total co2 flux from vegetation
  vegn_fco2 = -vegn%nep + vegn%csmoke_rate(isub_C) + sum(vegn%harv_rate(:,isub_C))
  ! --- convert it to kg CO2/(m2 s)
  vegn_fco2 = vegn_fco2*mol_CO2/(mol_C*seconds_per_year)

  ! --- update amount of nitrogen going out of the system  
  if (do_nitrogen) then
     vegn%nitrogen_out = vegn%nitrogen_out &
          + sum(vegn%harv_rate(:,isub_N))*dt_fast_yr &
          + vegn%csmoke_rate(isub_N)*dt_fast_yr
  endif

  ! --- accumulate values for climatological averages
  vegn%tc_av     = vegn%tc_av + cana_T
  vegn%tsoil_av  = vegn%tsoil_av + soil_ave_temp(soil,soil_carbon_depth_scale,WGHT_EXPONENTIAL)
  vegn%precip_av = vegn%precip_av + precip
  vegn%theta_av  = vegn%theta_av + soil_ave_theta(soil,depth_ave,WGHT_EXPONENTIAL)

  vegn%n_accum   = vegn%n_accum+1
  if(is_watch_point()) then
    __DEBUG3__(vegn%nep,vegn%npp,vegn%rh)
  endif

  !nitrogen conservation step 2
  ntot1 = vegn_tile_nitrogen(vegn) + vegn%nitrogen_out - vegn%nitrogen_in
  !call check_conservation('vegn_step_3','nitrogen',ntot0,ntot1,epsilon,lnd%time)
...
subroutine update_vegn_slow( )
  ...
  do while ( ce /= te )
     ...
     ! conservation check part I
     ntot0 = land_tile_nitrogen(tile) + tile%vegn%nitrogen_out - tile%vegn%nitrogen_in
...
!!$        call calc_miami_npp(tile%vegn)
     ...
     if (day1 /= day0 .and. do_cohort_dynamics) then
        ...
        call send_tile_data(id_ngain,sum(tile%vegn%cohorts(1:n)%ngain),tile%diag)
        call send_tile_data(id_nwoodgain,sum(tile%vegn%cohorts(1:n)%nwoodgain),tile%diag)
     ...
     if (year1 /= year0) then
        ...
        do isub = 1, n_substances
           where(harvest_spending_time(:)>0)
              tile%vegn%harv_rate(:,isub) = &
                   tile%vegn%harv_pool(:,isub)/harvest_spending_time(:)
           elsewhere
              tile%vegn%harv_rate(:,isub) = 0.0
           end where
        enddo
     endif
     ...
     do isub = 1,n_substances
       do ii = 1,N_HARV_POOLS
         call send_tile_data(id_harv_pool(ii,isub),tile%vegn%harv_pool(ii,isub),tile%diag)
         call send_tile_data(id_harv_rate(ii,isub),tile%vegn%harv_rate(ii,isub),tile%diag)
       enddo
       call send_tile_data(id_t_harv_pool(isub),sum(tile%vegn%harv_pool(:,isub)),tile%diag)
       call send_tile_data(id_t_harv_rate(isub),sum(tile%vegn%harv_rate(:,isub)),tile%diag)
       call send_tile_data(id_csmoke_pool(isub),tile%vegn%csmoke_pool(isub),tile%diag)
       call send_tile_data(id_csmoke_rate(isub),tile%vegn%csmoke_rate(isub),tile%diag)
     enddo
     ...
     call send_tile_data(id_nl,      sum(tile%vegn%cohorts(1:n)%nl),     tile%diag)
     call send_tile_data(id_nlv,     sum(tile%vegn%cohorts(1:n)%nlv),    tile%diag)
     call send_tile_data(id_nr,      sum(tile%vegn%cohorts(1:n)%nr),     tile%diag)
     call send_tile_data(id_nsw,     sum(tile%vegn%cohorts(1:n)%nsw),    tile%diag)
     call send_tile_data(id_nwood,   sum(tile%vegn%cohorts(1:n)%nwood),  tile%diag)
     call send_tile_data(id_nstore,  sum(tile%vegn%cohorts(1:n)%nstore), tile%diag)
     call send_tile_data(id_nstore_opt,sum(tile%vegn%cohorts(1:n)%nstore_opt),tile%diag)  
     call send_tile_data(id_rsn,     tile%vegn%res_soil_N,               tile%diag)     
     call send_tile_data(id_ssn,     tile%vegn%slow_soil_N,              tile%diag)     
     call send_tile_data(id_fln,     tile%vegn%fast_litter_N,            tile%diag)     
     call send_tile_data(id_sln,     tile%vegn%slow_litter_N,            tile%diag)     
     call send_tile_data(id_ntot,    land_tile_nitrogen(tile),           tile%diag)
     ...
     call send_tile_data(id_fln_in,  tile%vegn%fln_in,  tile%diag)
     call send_tile_data(id_sln_in,  tile%vegn%sln_in,  tile%diag)

     call send_tile_data(id_pr,      tile%vegn%cohorts(1)%Pr,  tile%diag)
     call send_tile_data(id_pl,      tile%vegn%cohorts(1)%Pl,  tile%diag)
     call send_tile_data(id_psw,     tile%vegn%cohorts(1)%Psw,  tile%diag)
     call send_tile_data(id_psw_alphasw, tile%vegn%cohorts(1)%Psw_alphasw, tile%diag)
    
     ! rates of input to fast and slow soil nitrogen. This can be easily moved
     ! to fast time steps, if desired
     call send_tile_data(id_fln_in_rate, (tile%vegn%fln_in-tile%vegn%fln_in0)/dt_slow_yr, tile%diag)
     call send_tile_data(id_sln_in_rate, (tile%vegn%sln_in-tile%vegn%sln_in0)/dt_slow_yr, tile%diag)
     ! reset fln_in0 and sln_in0 for the next time step; see NOTE in vegn_tile.F90 
     ! about how this works and why.
     tile%vegn%fln_in0=tile%vegn%fln_in
     tile%vegn%sln_in0=tile%vegn%sln_in
     ...
     ntot1 = land_tile_nitrogen(tile) + tile%vegn%nitrogen_out - tile%vegn%nitrogen_in
     !call check_conservation('update_vegn_slow','nitrogen',ntot0,ntot1,epsilon,lnd%time)
...
subroutine vegn_seed_transport()
  ...
  real :: total_bliving0, total_nliving0
  real :: total_bliving1, total_nliving1
  real :: unit

  unit = stdout()

  ! + conservation checking code, part 1 
  ce = first_elmt(lnd%tile_map, lnd%is, lnd%js) ; te = tail_elmt(lnd%tile_map)
  total_bliving0 = 0 ; total_nliving0 = 0
  do while ( ce /= te )
     call get_elmt_indices(ce,i,j)
     tile => current_tile(ce) ; ce=next_elmt(ce)
     if(.not.associated(tile%vegn)) cycle ! skip the rest of the loop body

     total_bliving0 = total_bliving0 + vegn_tile_bliving(tile%vegn)*tile%frac*lnd%garea(i,j)
     total_nliving0 = total_nliving0 + vegn_tile_nliving(tile%vegn)*tile%frac*lnd%garea(i,j)
  enddo
  call mpp_sum(total_bliving0)
  call mpp_sum(total_nliving0)
  ! - conservation checking code, part 1
  ...
  if (mpp_pe()==mpp_root_pe()) then
     write(unit,'(2(a,g24.16))')'SEED TRANSPORT: TOTAL_SEED_DEMAND=',total_seed_demand, &
         ' TOTAL SEED SUPPLY=',total_seed_supply
  endif
  ...
  if (mpp_pe()==mpp_root_pe()) then
     write(unit,'(2(a,g24.16))')'SEED TRANSPORT: f_demand=',f_demand, &
         ' f_supply=',f_supply
  endif
  ...
  ce = first_elmt(lnd%tile_map, lnd%is, lnd%js) ; te = tail_elmt(lnd%tile_map)
  do while ( ce /= te )
     ! set this point coordinates as current for debug output (vegn_add_bliving
     ! sometimes prints debugging information)
  ...
  ! + conservation checking code, part 2
  ce = first_elmt(lnd%tile_map, lnd%is, lnd%js) ; te = tail_elmt(lnd%tile_map)
  total_bliving1 = 0 ; total_nliving1 = 0
  do while ( ce /= te )
     call get_elmt_indices(ce,i,j)
     tile => current_tile(ce) ; ce=next_elmt(ce)
     if(.not.associated(tile%vegn)) cycle ! skip the rest of the loop body

     total_bliving1 = total_bliving1 + vegn_tile_bliving(tile%vegn)*tile%frac*lnd%garea(i,j)
     total_nliving1 = total_nliving1 + vegn_tile_nliving(tile%vegn)*tile%frac*lnd%garea(i,j)
  enddo
  call mpp_sum(total_bliving1)
  call mpp_sum(total_nliving1)
  write(unit,'(3(a,g24.16))') 'SEED TRANSPORT CHECK: BLIVING BEFORE=',total_bliving0, &
          ' AFTER=', total_bliving1,' DIFFERENCE=', total_bliving1-total_bliving0
  write(unit,'(3(a,g24.16))') 'SEED TRANSPORT CHECK: NLIVING BEFORE=',total_nliving0, &
          ' AFTER=', total_nliving1,' DIFFERENCE=', total_nliving1-total_nliving0
  ! - conservation checking code, part 2
...
#define DEFINE_VEGN_ACCESSOR_2D(xtype,x) subroutine vegn_ ## x ## _ptr(t,p);\
type(land_tile_type),pointer::t;xtype,pointer::p(:,:);p=>NULL();if(associated(t))then;if(associated(t%vegn))p=>t%vegn%x;endif;end subroutine
...
DEFINE_VEGN_ACCESSOR_0D(real,fast_litter_C)
DEFINE_VEGN_ACCESSOR_0D(real,fast_litter_N)
DEFINE_VEGN_ACCESSOR_0D(real,fast_litter_fsol)
DEFINE_VEGN_ACCESSOR_0D(real,slow_litter_C)
DEFINE_VEGN_ACCESSOR_0D(real,slow_litter_N)
DEFINE_VEGN_ACCESSOR_0D(real,slow_litter_fsol)
DEFINE_VEGN_ACCESSOR_0D(real,fast_soil_C)
DEFINE_VEGN_ACCESSOR_0D(real,slow_soil_C)
DEFINE_VEGN_ACCESSOR_0D(real,slow_soil_N)
DEFINE_VEGN_ACCESSOR_0D(real,slow_soil_fsol)
DEFINE_VEGN_ACCESSOR_0D(real,res_soil_C)
DEFINE_VEGN_ACCESSOR_0D(real,res_soil_N)
DEFINE_VEGN_ACCESSOR_0D(real,amm)
DEFINE_VEGN_ACCESSOR_0D(real,nitr)
...
DEFINE_VEGN_ACCESSOR_0D(real,slc_in_eq)
DEFINE_VEGN_ACCESSOR_0D(real,ssc_in_eq)
DEFINE_VEGN_ACCESSOR_0D(real,resc_in_eq)
DEFINE_VEGN_ACCESSOR_0D(real,sln_in_eq)
DEFINE_VEGN_ACCESSOR_0D(real,A_eq)
DEFINE_VEGN_ACCESSOR_0D(real,k_ls_eq)
DEFINE_VEGN_ACCESSOR_0D(real,k_ss_eq)

DEFINE_VEGN_ACCESSOR_1D(real,fsc_pool)
DEFINE_VEGN_ACCESSOR_1D(real,fsc_rate)
DEFINE_VEGN_ACCESSOR_1D(real,ssc_pool)
DEFINE_VEGN_ACCESSOR_1D(real,ssc_rate)
DEFINE_VEGN_ACCESSOR_1D(real,csmoke_pool)
DEFINE_VEGN_ACCESSOR_1D(real,csmoke_rate)

DEFINE_VEGN_ACCESSOR_2D(real,harv_pool)
DEFINE_VEGN_ACCESSOR_2D(real,harv_rate)
...
DEFINE_COHORT_ACCESSOR(real,nl)
DEFINE_COHORT_ACCESSOR(real,nr)
DEFINE_COHORT_ACCESSOR(real,nlv)
DEFINE_COHORT_ACCESSOR(real,nsw)
DEFINE_COHORT_ACCESSOR(real,nwood)
DEFINE_COHORT_ACCESSOR(real,nliving)
DEFINE_COHORT_ACCESSOR(real,nstore)
DEFINE_COHORT_ACCESSOR(real,nstore_opt)
DEFINE_COHORT_ACCESSOR(real,nfix_per_bliv)
DEFINE_COHORT_ACCESSOR(real,nfixrate)
