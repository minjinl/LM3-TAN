...
character(len=*), parameter :: &
     version = '$Id: vegn_cohort.F90,v 18.0.2.1.2.2.2.2 2011/04/29 15:39:57 slm Exp $', &
...
! ---- nutrient content, corresponding to the biomass compartments
  real    :: nl, nlv, nr, nsw, nwood, nstore, nstore_opt
  real    :: nliving
  real    :: nsf = 1.0 ! measure of the fullness of extra N: ratio of current N storage to its optimal value
  real    :: nfixrate  = 0.0 ! nitrogen fixation rate, kgN/(m2 year)(are units correct?)
  real    :: nfix_per_bliv
  real    :: ngain     = 0.0 ! nitrogen gain between vegn_growth calls
  real    :: nwoodgain = 0.0 ! wood nitrogen gain between vegn_growth calls 
...
! ---- auxiliary variables 
  ...
  real :: carbon_gain = 0.0 ! carbon gain during the day, kg C/m2
  real :: carbon_loss = 0.0 ! carbon loss during the day, kg C/m2, diag only
  real :: bwood_gain  = 0.0 ! heartwood gain during the day, kg C/m2

  ! used in fast time scale calculations
  real :: npp_previous_day
  real :: npp_previous_day_tmp = 0.0
  real :: npp_previous_day_pot
  real :: npp_previous_day_pot_tmp = 0.0
