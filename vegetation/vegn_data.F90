...
public :: &
    ...
    SOMinput_frac_slow, SOMinput_frac_res, &
    k_litter_fast, k_litter_slow, k_SOM_slow, k_SOM_res
integer, public :: isub_C = -1 ! substance index for carbon
...
character(len=*), parameter   :: &
     version     = '$Id: vegn_data.F90,v 18.0.2.3 2010/05/02 21:22:57 slm Exp $', &
...
!!$  real    :: rtrans       ! fine root translocation fraction
  ...
  ! nitrogen data -- may be they don't belong there? may be they should be
  ! in a separate data structure? but for now it looks like a natural place
  ! for species-dependent parameters
  real    :: retrans_n   ! fraction of Nitrogen re-translocated
  real    :: CtoN(NCMPT) ! stoichiometric C:N ratio in carbon compartments
  real    :: knup_plant  ! N uptake rate constant of plants
  real    :: tau_fix     ! relaxation time for nitrogen fixation, units=???
end type
...
! nitrogen-related vegetation parameters
real :: a_lf = 0.86, b_lf = 0.018, f_lf_min = 0.33 ! parameters for distribution 
         ! between fast and slow litter pools
real :: lignin_lf = 0.2 ! lignin content of plant litter-fall
real :: SOMinput_frac_slow = 0.5 ! fraction of litter residue into Soil Organic Matter
real :: SOMinput_frac_res  = 0.0025 ! fraction of litter residue into Soil Organic Matter
real :: k_litter_fast = 10.0   ! decomposition rate of fast litter, 1/year
real :: k_litter_slow = 0.65   ! decomposition rate of slow litter, 1/year
real :: k_SOM_slow    = 0.1    ! decomposition rate of slow soil organic matter, 1/year
real :: k_SOM_res     = 0.0029 ! decomposition rate of resistant soil organic matter, 1/year
...
10   continue
  ...
  ! register carbon as one of the substances and cache its index
  call add_substance('C', 'carbon' )
  call inq_substance('C', idx=isub_C )
