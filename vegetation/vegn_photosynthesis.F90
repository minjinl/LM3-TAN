...
character(len=*), private, parameter :: &
   version = '$Id: vegn_photosynthesis.F90,v 17.0.2.2 2010/04/30 00:59:19 slm Exp $', &
  ...
  case(VEGN_PHOT_LEUNING)
     if(cohort%lai > 0) then
        ...
        call gs_Leuning(PAR_dn, PAR_net, cohort%prog%Tv, cana_q, cohort%lai, &
             ...
             cohort%extinct, fs+fw, stomatal_cond, psyn, resp, cohort%pt, cohort%nsf)
        ! store the calculated photosynthesis and photo-respiration for future use
        ! in carbon_int
...
subroutine gs_Leuning(rad_top, rad_net, tl, ea, lai, leaf_age, &
  ...
  real,    intent(in)    :: nsf  ! nitrogen state
  ...
  real :: gsbar;
  real :: w_scale;
  real :: n_scale ! scaling factor due to nitrogen limitation
  ...
  ! nitrogen limitation
  if (do_nitrogen.and.do_nitrogen_plant_feedback) then
    n_scale = n_phot(nsf);
  else
    n_scale = 1.0;
  endif
